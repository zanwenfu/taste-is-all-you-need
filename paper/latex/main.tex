...
\begin{figure}[t]\centering\includegraphics[width=0.66\linewidth]{fig_undercount.pdf}\caption{Every run that produced at least one regression event, in both GPT-5.6 sweeps. Left bars (red) are events recorded on the timeline; right bars (grey) are held-out test failures visible in the graded final patch.}\label{fig:fig-undercount}\end{figure}
...
\begin{figure}[t]\centering\includegraphics[width=0.6\linewidth]{fig_timeline.pdf}\caption{One run that the official grader marked resolved, with all 147 graded tests passing. The timeline shows three regressions of the same test function, each repaired by rollback two observations later.}\label{fig:fig-timeline}\end{figure}
...
\begin{figure}[t]\centering\includegraphics[width=0.76\linewidth]{fig_contrast.pdf}\caption{The five recovery arms under the GPT-5.6 stack on the same 40 instances. Contamination counts cells whose final patch fails a previously-passing test.}\label{fig:fig-contrast}\end{figure}
