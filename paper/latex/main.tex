...
\textbf{Agent scaffolds and recovery.} SWE-agent \cite{ref9}, OpenHands \cite{ref10}, and Agentless \cite{ref11} established the design space of coding-agent scaffolds; mini-swe-agent \cite{ref12} reduced it to a bash-only loop. CodeAct \cite{ref13} and ReAct \cite{ref14} are the underlying action formalisms. Within-episode recovery has been studied as verbal self-reflection [15, 16], progress-gated recovery \cite{ref17}, checkpoint repair for program-of-thought \cite{ref18}, and provenance-based rollback \cite{ref19}. Operating-system framings for agents \cite{ref20} treat checkpointing and scheduling as shared primitives. Process reward models and rubric verifiers for software agents [24, 25] address the verifier-precision problem we encounter in Section 5.5 from the training side.

\textbf{Evaluation infrastructure.} Trajectory-level diagnostics \cite{ref22} and holistic leaderboards \cite{ref21} argue that resolve rate alone is uninformative; long-horizon benchmarks \cite{ref23} make the same point with harder tasks. Our instrument is a specific, validated trajectory-level measure of one quantity.

\textbf{Automated program repair.} The regression problem is the plausible-versus-correct patch problem of automated program repair [26, 28, 29]. A study of two million patches \cite{ref27} showed that regression tests are the primary defence against overfitting patches; our results concern what happens to regressions that the agent's own process repairs before the patch is ever tested.
...
\begin{figure}[t]\centering\includegraphics[width=\linewidth]{fig_contrast.pdf}\caption{The three recovery policies under the GPT-5.6 stack on the same 40 instances.}\label{fig:fig-contrast}\end{figure}
...
\appendix
\section{A: The measurement defect catalogue}

\subsection{A.1 The catalogue by mechanism}
...
\subsection{A.2 The defect that invalidated a conclusion}
...
\subsection{A.3 Mechanisms in public infrastructure}
...
\appendix
\section{B: Pre-declaration and analysis}
