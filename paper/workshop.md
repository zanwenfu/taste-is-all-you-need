...
Figure 2 shows every run that produced a regression event, under the rollback policy with the GPT-5.6 stack. The timeline recorded 184 declared events across eight runs. The final patch exposed one. Every other event was created and then removed by the harness's rollback before the run ended, so the graded patch was clean. The same measurement made from the final patch, which is how prior work measures regressions [8], would report one event in eight runs.
...
A fourth arm replaces the monitor's planner-written check with the repository's previously-passing tests, run in the agent's container at the start of the run and after every attempt; a step is rejected only if a test that passed at the start fails now. Everything else (policy, model, instances, caps) is unchanged. It resolved **26 of 40 instances (65.0%)**, the highest of any arm, and left **no contaminated final tree** (Figure 4). Paired against plain rollback on the 35 shared instances, it resolved 10 that plain rollback did not and lost 1 (McNemar exact p = 0.012). Regression exposure was unchanged (70 declared events in 5 bearing runs; co-primary p = 0.73): the gate does not stop the model from breaking adjacent behaviour, it catches the breakage with the tests that define it and resets. The three gated runs that failed each failed on a genuine regression the gate refused (8, 2, and every test), and the repository family that had tripped the plain arm's circuit breaker completed. The suite runs cost $2.37 over plain rollback.

## 6. Discussion

**Implications for agent-OS design.** If checkpointing and recovery become shared primitives, timeline observability must be one too: a recovery primitive without an observation primitive makes runs look cleaner while hiding the activity that distinguishes policies. The 184-to-1 ratio is what any final-state grader will report for any harness that repairs its own mistakes.

**On the measurement defects.** Building the instrument produced 28 defects (Appendix A); 23 produced a plausible number rather than an error, and 27 were present while a passing test suite watched. They fall into four mechanisms, each with public instances in benchmark infrastructure (Appendix A.3). Two cheap rules prevented most of them: type infrastructure failure as a missing observation, never as a verdict; and require the instrument to prove it is alive, not merely that it reported nothing.

## 7. Limitations

All measurements come from a 40-instance development slice, one sweep per arm and per stack; run-to-run variability is unmeasured, and the confirmatory design specifies seeded replications on held-out instances. The cross-stack comparison cannot separate a model from its provider adapter. The held-out tests observe roughly the gold test patch's blast radius, so event counts are lower bounds. Confidence intervals treat the fixed slice as the population and ignore repository clustering (46% one repository). The harness is our own; the instrument has not yet been applied to a public scaffold.

## 8. Conclusion

Final-state evaluation of coding agents is blind to regressions that the agent's own process repairs, and in our measurements that is nearly all of them. Measuring the timeline is feasible, validates against controls, and separates recovery policies that the final patch cannot distinguish. It also exposes the cost of recovery, and pays it back: a rollback triggered by the repository's own tests kept every final tree clean while resolving more tasks than any other policy.
