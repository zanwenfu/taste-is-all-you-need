...
![The harness and the instrument. The test row shows a previously-passing test regressing at s₃–s₄ and repaired by a rollback to s₂, which the official grader, reading only sₙ, never sees. The counts are the GPT-5.6 rollback arm's, over all 47 of its runs.](fig_system.pdf){width=0.54}
