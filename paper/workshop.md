...
**Disclosure.** The first unblinding of this comparison gave p = 0.375 because seven catastrophically contaminated cells had been dropped as ungradable; Appendix B records the grading correction and the re-grade of their archived trees.
...
A fourth arm, added after the contrast was unblinded and therefore exploratory, replaces the monitor's planner-written check with the repository's previously-passing tests, run in the agent's container at the start of the run and after every attempt; a step is rejected only if a test that passed at the start fails now. It resolved **26 of 40 instances (65.0%)**, the most of any arm, with **no contaminated final tree** (Figure 4). Paired against plain rollback on the 35 shared instances, it resolved 10 that plain rollback did not and lost 1 (McNemar exact p = 0.012; post hoc, one of seven tests reported, unadjusted). Paired onset exposure did not differ detectably (p = 0.73); the gated arm's 70 declared events against 140 is a storm-dominated count we do not treat as a finding. The gate runs the test files that hold the instance's previously-passing tests, selected from benchmark metadata, and its baseline oracle coincides with the grading oracle (median ratio 1.00), so the clean final trees are guaranteed by construction and the informative result is the resolve gain; a split variant that watches half of each instance's test files and is graded on the other half is the next experiment. Its three failures were genuine regressions the gate refused. A second full sweep of plain rollback reproduced the first (13 and 13 resolved on 34 paired instances; 6 of 37 bearing runs both times), so the gated gain is well outside run-to-run variability.

## 6. Discussion

**Why timeline regressions matter.** A regression the agent repairs before submission did not reach the user, so the objection that it is harmless deserves an answer. First, it is not free: across the bearing runs, 27% of spend ($0.63 of $2.33) was consumed while a regression was open, that is, on work done in a broken tree. Second, the events are the training signal process reward models for software agents [24, 25] are looking for, with a ground-truth label per step rather than per episode. Third, the transient states include destructive ones (a tree with every test uncollectable) that a final-state grade never records. And for agent-OS design, a recovery primitive without an observation primitive hides exactly the activity that distinguishes recovery policies.
...
**Summary.** Final-state evaluation misses the regressions an agent's own process repairs, which in our measurements is nearly all of them. Measuring the timeline is feasible, validates against controls, and separates recovery policies the final patch cannot; it also locates rollback's cost in verifier precision, which a verifier built from the same tests removes.

## 7. Limitations

All measurements come from a 40-instance development slice: one sweep per arm, plus a calibration and one replication of plain rollback for the GPT-5.6 stack, so variability is measured for that arm only; the confirmatory design specifies seeded replications on held-out instances. The cross-stack comparison is a single pairwise test on six bearing runs, confounded with the provider adapter; the gated arm is post hoc. The held-out tests observe roughly the gold test patch's blast radius, so event counts are lower bounds; confidence intervals ignore repository clustering (16 of 40 instances are django); and the instrument has not been applied to a public scaffold, which is the next experiment.
