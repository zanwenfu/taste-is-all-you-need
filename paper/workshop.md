...
*Submission draft v11. The converter drops this line. Arms are named (rollback, repair-in-place, no recovery).*

---

## Abstract

Coding agents are graded on the final patch alone, so nobody knows what they break in between. We commit the working tree after every mutating tool call and replay the benchmark's previously-passing tests at every commit inside its pinned container, then apply this to mini-swe-agent 2.4.6 unmodified: its own loop, prompts, model layer, budgets and submission protocol, with only its environment object replaced.

Agents break existing tests often and almost always repair them. Across 80 runs on 40 SWE-bench Verified tasks, resolving 34 and 33 of 40 under two model families, 11 runs broke 135 previously-passing test functions, 100 of them distinct. The official grader, reading only the final tree, saw one. Neither did the agent: over the window each test was broken, no test command ran it in 6 of the 10 classifiable cases, so a method reading only the agent's logs is blind to most of them. The undercount holds in our own harness and on SWE-bench Live.

The instrument is also a control. Replacing our harness's model-written check with one built on those same tests takes resolve from 13 to 26 of 40 with no contaminated final tree, at 16 to 32 seconds of added wall-clock, though it is exploratory and circular by construction. Two things bound all of this: rollback's clean trees are partly empty ones, since 17 of 37 runs ship a tree they never changed, and nine of the ten trees that killed the suite opened no timeline event at all. We will release the instrument, its validation protocol, and the archived timelines. Appendix A catalogues 28 measurement defects met while building it, 23 of which produced plausible numbers rather than errors.
...
The answer is not what we expected. Agents break the oracle often and almost always repair it, so what the endpoint misses is transient. The agent's own tests and its action log miss it too. We measure the timeline instead of the endpoint. Every mutating tool call becomes a commit on a git reference the agent cannot see, and the oracle is re-run at every commit inside the benchmark's pinned container. An agent operating system needs this observation primitive before it can build a recovery primitive. We contribute:

1. **An instrument for event-level regression measurement**, applicable to an agent we did not write. Every mutating tool call becomes a commit on a hidden git reference, and the benchmark's previously-passing oracle is replayed at every recorded state inside its pinned container (Section 3).
...
3. **The same measurement across recovery policies.** The undercount holds in our harness and on SWE-bench Live. But nearly half the rollback arm's runs ship a tree they never changed, so a clean final tree partly means no edits at all (Sections 5.2, 5.3, 5.5).
4. **Recovery policy depends on the monitor's precision.** With a model-written check, rollback buys 4 fewer contaminated trees for 11 fewer resolved instances (1 against 5; 13 against 24). Replacing that check with the repository's own oracle gets both: 26 resolved, no contaminated tree, and a lower cost per resolved instance than either. It is exploratory and circular by construction (Section 5.4).
5. **A catalogue of 28 measurement defects** found while building the instrument (Appendix A); 23 produced a plausible number rather than an error.

All measurements are exploratory, on development slices excluded from future confirmatory study.

## 2. Background and related work

**Benchmarks and their validity.** SWE-bench [1] grades a patch by tests that must go from failing to passing and tests that must keep passing. Verified [2] is the human-screened subset, deprecated as a capability metric [45]. Concerns about it have accumulated. They include contamination and memorisation [6], plausible-but-wrong patches overstating resolve by 6.2 points [36], and insufficient tests, which UTBoost [7] augmented to expose mislabelled patches affecting 24.4% of leaderboard entries. Rolling [3, 5], private [4], and long-horizon [23] alternatives came in response. TDAD [8] measured regressions in *submitted* patches on Verified (6.1% of previously-passing tests, 30.2% of non-empty patches) and reduced them with an impact-analysis map. Process-level benchmarks score intermediate states from logs [34, 35], and AgentLens [37] names regression cycles in 10.7% of passing OpenHands runs, against our 11 of 80 runs, or 29% of the 38 that made more than two tree-changing observations. A log-derived rate is bounded below by what the agent chose to run, and in 6 of our 10 classifiable cases it ran nothing that would have executed the broken test. We execute the oracle at every mutating call instead.

**Agent scaffolds and recovery.** SWE-agent [9], OpenHands [10], Agentless [11], and mini-swe-agent [12] span the scaffold design space [13, 14]. Within-episode recovery appears as self-reflection [15, 16], progress-gated recovery [17], checkpoint repair [18], and aligned checkpoints [40]. Provenance-based recovery is surveyed in [19]. Closest to us, Kim et al. [38] match logged edits against the gold diff and recover agents that reach a gold-identical patch and then destroy it. Gao et al. [39] bind verifier evidence to code states. Neither counts regressions or compares policies. Running the repository's tests during a run is deployed practice [42, 43]. Operating-system framings [20] and the branch-context primitive [41] supply scheduling and fork, commit, and abort, but do not observe the tree between actions. The regression problem is program repair's plausible-versus-correct patch problem [26, 28, 29], where regression tests are the main defence against overfitting [27]. Trajectory-level evaluation [21, 22] argues resolve rate alone is uninformative, diagnosing structure from recorded actions. An action log cannot show a test the agent never ran. Public leaderboard trajectories record actions but not the tree at each step, which the instrument commits.

## 3. Instrument

![The harness and the instrument. The test row shows a previously-passing test regressing at s₃–s₄ and repaired by a rollback to s₂, which the official grader, reading only sₙ, never sees. The counts are the GPT-5.6 rollback arm's, over all 47 of its runs.](fig_system.pdf){width=0.85}
...
**Validation.** Five checks run before any paid experiment (negative and positive controls, the latter including *recovered* regressions, a flake screen, an unknown-rate ceiling, a baseline liveness check), plus a golden check driving the gold patch through the agent's real tool path and a re-scoring control that re-measures an archived timeline with no model calls. Re-scoring four arms' bearing runs reproduced their episode counts and probe sets exactly. Appendix C gives the detail.

## 4. Experimental setup

**Benchmark and slice.** SWE-bench Verified is a measurement substrate here, not a leaderboard. Its pinned images and oracles make the measurement possible. A 40-instance development slice (16 from django), stratified and fixed before any measurement, was used throughout and is excluded from future confirmatory study. The GPT-5.6 stack also ran its first 10 as a calibration, and plain rollback ran twice. SWE-bench Live [3], built monthly from live repositories, is the second substrate, graded by its own harness whose rules we mirror. Its slice was fixed before any outcome (Appendix B) and its oracles are about twenty times larger (median 1,189 against 58 tests).

**Harness.** A planner decomposes the task into steps with verification commands, a worker executes each with three tools (read, write, shell), and a monitor runs the verification. On failure the recovery policy acts: **rollback** (reset to the last verified checkpoint and retry), **repair-in-place** (retry from the failed tree), or **no recovery** (the FAIL is final and the run ends). Runs have a $4 work-cost cap. One instance under one policy is a cell. Each (stack, policy, substrate) triple is an arm.

**Public scaffold.** mini-swe-agent 2.4.6 [12] runs unmodified (its own loop, prompts, model layer, 250-step and $3 caps, and submission protocol) with only its environment object replaced, so every bash command executes in the benchmark's pinned container and the tree is committed to the hidden reference after each. The seam is inert but not invisible. The agent's `git diff` and `git status` show its own uncommitted edits and none of the instrument's commits, but the synthetic baseline carries a marker name 54 of the 80 runs saw, and those runs broke tests more often, though confounded with editing more (Appendix C). Appendix C lists six departures from its leaderboard configuration, and a grid census there puts its sampling inside our arms' range: a median of 3 and 2 tree-changing observations per run against our 2 to 5, at 9 to 15 bash commands each. We claim no equivalence to its published resolve rate, and because its budget differs in kind from our cap, no comparison between harnesses is tested.

**Models.** Two frontier stacks under identical harness, policy, instances and caps: `claude-opus-4-7`/`claude-sonnet-4-6` and `gpt-5.6-sol`/`gpt-5.6-terra` (planner/worker).

**Outcomes.** Resolve is the official grader's verdict on the final patch. Events are reported at three levels because a few runs produce most: incidents (observations at which at least one oracle test broke), declared events (one per test function and onset, parametrised variants collapsed), and bearing runs (runs with at least one event). Incident *exposure*, a run's incident count, is the pre-declared co-primary. An event and a final-state failure are different objects, so the undercount is in matched units (test functions broken during a run against those still failing at the end). Contamination counts oracle tests failing in the graded patch, net of baseline-dead. A tree that kills the suite is graded as failing every test but is a missing observation for event counting, so a contaminated cell need not be a bearing run.

**Pre-declaration.** The event unit, the proceed criteria, and the recovery-policy contrast were declared before the relevant data existed, and the analysis was committed before either comparison arm finished. One amendment preceded unblinding and one grading correction followed it (Section 5.3, Appendix B).
...
The timeline recorded 56 events in 5 of the 40 Claude runs and 79 in 6 of the 40 GPT-5.6 runs, on 8 instances, 3 bearing under both. Those 11 runs broke 135 oracle test functions counted per run (100 distinct, because the stacks overlap on the three shared instances). The official grader, reading only the final tree, caught exactly one of them: `test_pkgfile` in pytest-6197 (Claude 0 of 56, GPT-5.6 1 of 79). The scaffold has no monitor or rollback, so every closure is an ordinary forward edit. The eleven stayed open for 1 to 22 bash commands (median 4), and three runs supply 81% of the 135. No run making two or fewer tree-changing observations (run and final excluded) was bearing, and 11 of the 38 that made more broke something (29%). Much of what the endpoint misses is a transient inside one logical edit, but transients are not free: 14% of what these runs cost ($1.29 of $9.52) was spent while a regression was open.

**One run in full.** On django-12965 the GPT-5.6 scaffold read `SQLDeleteCompiler` for twelve commands, then at command 13 changed `single_alias` to count active aliases with `<= 1`. Its own reproduction script crashed, and at command 15 `runtests.py delete` printed `ERROR: test_fast_delete_large_batch`. Command 16 replaced the edit, command 18 ran the suite clean, and the run graded resolved, 52 of 52 previously-passing tests. The timeline holds all of this. The graded patch holds none of it.

**Would its own testing have caught it?** For each breaking edit we read the trajectory over the window the test was broken, from the breaking command to just before the repair, asking whether a test runner ran, whether any run would have executed *this* test, and whether its output showed the failure. Coverage is three-valued, with *unknown* for runs that stopped early or were truncated (Appendix C). In 6 of the 10 classifiable cases the agent's own test commands never ran the broken test. In the other 4 they ran it and printed the failure by name, and it was repaired anyway. The eleventh is unknown, a `pytest -x` run that stopped earlier. Self-testing is blind to the first group and ignored on the second. The endpoint sees neither. With eleven cases this is a case series, not a rate.

### 5.2 The same undercount in our harness

![Every run that broke an oracle test, in matched units. Test functions broken on the timeline, dark where still failing at grade time. Left, the unmodified public scaffold under both models. Right, our rollback arm, hatched where the final tree is byte-identical to its start.](fig_undercount.pdf){width=0.66}

Eight of the 47 GPT-5.6 rollback runs (the 37-run sweep and a 10-instance calibration) broke a previously-passing test, 146 test functions counting each run separately (145 distinct). One of those, `test_can_read_toml_env_variable` in pylint-6386, was still failing at grade time. Three storms supply 88% of the 184 declared events over all 47 runs. Table 1's 140 events and 104 test functions are the sweep alone, and declared events are exposure only.

**Attrition and rates.** Under rollback the Claude stack resolved 13 of 40 (32.5%, exact 95% CI 18.6% to 49.1%) for $34.24 and the GPT-5.6 stack 13 of 40 for $8.14. A circuit breaker stopped three cells and a synchronisation defect lost two, so 37 were attempted and 35 graded (13 of 35, 37.1%). Rates are out of 40 unless stated, and one instance is unresolvable under the current parser (Appendix A.3), putting the ceiling at 39 for every arm, the scaffold included.

A clean tree does not always represent work. Excluding the hidden reference, 17 of the 37 rollback runs end byte-identical to their start. Four of the six bearing sweep runs shipped nothing. Over the two that delivered a patch, 1 of 55 broken test functions is still failing, so the undercount rests on the public-scaffold arm, where no run ends unchanged.

**Model stack.** Event frequency also differed by model stack, but does not generalise. Under the same harness and policy the Claude stack produced no events in 40 runs against the GPT-5.6 stack's 140 in 6 of 37 (one-sided Fisher p = 0.010; reclassify three of the six as non-bearing and p = 0.11), yet the same `claude-sonnet-4-6` worker under unmodified mini-swe-agent is bearing on 5 of 40 with 56 events, and the difference is absent on Live. Model and adapter are confounded, and controls are in Appendix C.
...
### 5.3 Recovery policy: what the clean final tree costs
...
Replacing the planner-written check with the repository's own oracle takes the GPT-5.6 stack's resolve from 13 of 40 to **26 of 40 (65.0%)** with **no contaminated final tree**, against the ungated arms' 24 and 22, and 4 empty patches of 40 against rollback's 17 of 37. A step is rejected only if a test that passed at the start now fails. This fourth arm was added after unblinding and is exploratory. The gate runs at least 4 times in the median run at 4 to 8 seconds each, so 16 to 32 seconds of wall-clock and $2.37 more model spend than rollback, and per resolved instance it costs $0.40 against rollback's $0.63, repair-in-place's $0.61 and no recovery's $0.20 with nine contaminated trees. The split gate leaves none at $0.34. It beat rollback on 35 shared instances, winning 10 discordant pairs and losing 1 (p = 0.012), and left no contaminated tree against repair-in-place's 5.

The gate gives a guarantee, not a rate, and it is circular. Its tests are the benchmark's own oracle (median ratio 1.00), so a clean tree and part of the resolve gain both follow whenever the gate saw the final tree and the grader reads what the gate read. A fifth arm weakens that, reading a deterministic half of the ids (2,278) and never the other half (2,585), on which the grader also rules [44]. The split is over ids, not files, so a break reaching a held-out id usually reaches a watched one, and the held-out half bounds the leak instead of sampling it. A file-level or non-oracle gate would settle it. It resolved **28 of 40 (70.0%)**, indistinguishable from the full gate (5 discordant pairs to 3, p = 0.73) and better than rollback (13 to 2, p = 0.007), with no held-out failure apart from the parser-capped instance.
...
Live keeps the undercount and loses the resolve rates, at most 6 of 40 even rot-aware. Rollback broke 147 test functions across 6 of 39 runs, none still failing, against Verified's 104 across 6 on oracles twenty times larger. But 25 of the 39 shipped an unchanged tree, so the clean result is partly no result. The gate contaminated two trees here, one of them after passing every check (Appendix C). No recovery left all 30 of its own (p = 0.125).

## 6. Discussion

**Why timeline regressions matter.** A repaired regression still costs money. Work done while one was open took 14% of the scaffold's bearing-run cost and 27% of our harness's ($0.63 of $2.33), both small-base estimates. Patch verifiers [24] and process reward models [25] need step-level ground truth, and instance-scale training sets [32, 33] do not carry it. The events supply it, and they tell a recovery primitive [41] when to abort. Two rules would have caught Appendix A's A and B classes, 14 of the 28: record infrastructure failures as missing observations, and require positive evidence that the measurement path is live. Two changes follow: a recovery monitor should run tests rather than trust a model-written check, and a benchmark should report what broke during the run beside the final verdict.
...
[34] M. B. Madiraju and M. S. P. Madiraju. RigorBench: Benchmarking engineering process discipline in autonomous AI coding agents. arXiv:2606.22678, 2026.
[35] J. Chen et al. SWE-CI: Evaluating agent capabilities in maintaining codebases via continuous integration. arXiv:2603.03823, 2026.
[36] Y. Wang, M. Pradel, and Z. Liu. Are "solved issues" in SWE-bench really solved correctly? An empirical study. ICSE 2026. arXiv:2503.15223.
[37] P. Sahoo et al. AgentLens: Revealing the lucky pass problem in SWE-agent evaluation. arXiv:2605.12925, 2026.
[38] M. Kim et al. Coherence collapse: Diagnosing why code agents fail after reaching the right code. arXiv:2603.24631, 2026.
[39] X. Gao, J. Yang, and Q. Yang. Looping is not reliability: State-bound evidence and typed revision contracts for agentic code repair. arXiv:2607.24604, 2026.
[40] Y. Zhuang et al. AgentRewind: Recoverable execution for long-horizon LLM agents. arXiv:2608.14380, 2026.
[41] C. Wang and Y. Zheng. Fork, explore, commit: OS primitives for agentic exploration. arXiv:2602.08199, 2026.
[42] Y. Chen, T. Ahmed, R. Jabbarvand, and M. Hirzel. Can old tests do new tricks for resolving SWE issues? FSE 2026. arXiv:2510.18270.
[43] P. Gao et al. Trae Agent: An LLM-based agent for software engineering with test-time scaling. arXiv:2507.23370, 2025.
[44] T. Ahmed, J. Ganhotra, A. Shinnar, and M. Hirzel. Investigating test overfitting on SWE-bench. arXiv:2511.16858, 2025.
[45] OpenAI. Why SWE-bench Verified no longer measures frontier coding capabilities. OpenAI blog, February 2026. openai.com/index/why-we-no-longer-evaluate-swe-bench-verified.
...
**Compute.** Every run, replay, and grade ran on one 12-vCPU, 22 GB x86 virtual machine running Docker, with no GPUs. The six harness arms on Verified in Table 1 cost $81.42 in model spend and the two public-scaffold arms $43.51, plus $6.87 for the second rollback sweep. Calibration and pilots cost a comparable amount, and the Live sweeps report their own spend in Table 1.
...
**Reading Table 1.** *Baseline-dead* tests already fail in the unmodified image. *Rot-aware* resolve counts a run resolved when every target test passes and no failure lies outside that set. On Live, **resolved** is of 40 while **empty** and **bearing** are of the 39 attempted (torchtune-1806 exceeds the replay budget). **broken → left** omits any cell whose suite the grader could not collect, since such a cell would otherwise read as perfect capture, so repair-in-place's row covers six of its seven bearing runs and 83 of its 84 events.

**Incident exposure.** The pre-declared co-primary is a run's incident count, not an arm's total; Table 1's **inc.** column gives the totals.
