...
**Evaluation infrastructure.** Trajectory-level diagnostics [22], holistic leaderboards [21], and long-horizon benchmarks [23] argue that resolve rate alone is uninformative. Our instrument is a validated trajectory-level measure of one quantity.

**Automated program repair.** The regression problem is the plausible-versus-correct patch problem of automated program repair [26, 28, 29]; a study of two million patches [27] found regression tests to be the main defence against overfitting patches. Our results concern regressions the agent's own process repairs before the patch is ever tested.

## 3. Instrument

![The harness and the instrument. The planner, worker, monitor, and recovery policy run inside the benchmark's pinned container; every mutating tool call is committed to a hidden git reference; held-out tests are replayed at every observation and detections are attributed by coverage. In the gated arm (dashed), the timeline's tests serve as the monitor. The official grader sees only the final patch.](fig_system.pdf)
