...
**Summary.** Final-state evaluation of coding agents misses the regressions that the agent's own process repairs, and in our measurements that is nearly all of them. Measuring the timeline is feasible, validates against controls, and separates recovery policies the final patch cannot distinguish; it also locates rollback's cost in verifier precision, and a verifier built from the same tests resolved more tasks than any other policy while keeping every final tree clean.
