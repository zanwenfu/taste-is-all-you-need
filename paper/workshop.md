...
![The harness and the instrument. The test row shows a previously-passing test regressing at s₃–s₄ and repaired by a rollback to s₂, which the official grader, reading only sₙ, never sees. The counts are the GPT-5.6 rollback arm's, over all 47 of its runs.](fig_system.pdf){width=0.62}
...
Event frequency also differed by model stack, but does not generalise. Under the same harness and policy the Claude stack produced no events in 40 runs against the GPT-5.6 stack's 140 in 6 of 37 (one-sided Fisher p = 0.010; reassign three bearing runs and p = 0.11), yet the same `claude-sonnet-4-6` worker under unmodified mini-swe-agent is bearing on 5 of 40 with 56 events, and the difference is absent on Live. Model and adapter are confounded, and controls are in Appendix C.
...
**What to build differently.** A recovery monitor should run tests, not trust a model-written check. A benchmark should report what broke during the run beside the final verdict.
