...
![Every run that produced at least one regression event in the 10-instance calibration and the first 40-instance GPT-5.6 rollback sweep: events recorded on the timeline (red) against previously-passing test failures visible in the graded final patch (grey).](fig_undercount.pdf){width=0.5}
...
Live keeps the undercount and loses both the resolve rates and the stack difference (Table 1). Resolve collapsed for every arm, to at most 6 of 40 even under the rot-aware count against 13 to 26 for the same four arms on Verified, so the recovery contrast on resolve is uninformative on Live; the collapse is consistent with training-data contamination of Verified [6, 46], though Live's larger oracles and 15 baseline-dead cells mean the substrates differ in more than contamination. The undercount replicated: GPT-5.6 rollback recorded 156 events in 9 incidents across 6 of 39 runs and left no contaminated final tree, while no recovery left one in each of its 4 bearing runs (worse than rollback on 4 instances, better on 0; sign test p = 0.125). The gated arm, clean by construction on Verified, left two contaminated trees on Live against rollback's none (sign test p = 0.50) and resolved 1 (4) against 0 (2): one run was cut off by the $4 cap before the gate saw its final tree; the other passed every gate check yet fails 13 tests under the grader, 7 in the file the hidden test patch rewrites. The stack difference did not: the Claude stack, silent on Verified, was bearing on 3 of the 39 shared Live instances (49 events) against GPT-5.6's 6 of 39 (one-sided Fisher p = 0.24). The Verified zero and Live's 3 of 39 are not distinguishable (the exact interval for 0 of 40 reaches 8.8%): memorised fixes on Verified and a cleaner stack on both substrates both fit.

## 6. Discussion

**Why timeline regressions matter.** A repaired regression is not free: 27% of spend across the eight Verified bearing runs ($0.63 of $2.33) went to work done while a regression was open. The events are the per-step ground truth patch verifiers [24] and process reward models [25] need; the timeline is what tells an agent-OS recovery primitive such as a branch context [42] when to abort. Two rules would have prevented most of the 28 defects met while building the instrument (Appendix A): record infrastructure failures as missing observations, and require positive evidence that the measurement path is live.

**Limitations.** Each substrate has a 40-instance slice and one sweep per arm; the cross-stack comparisons rest on six and nine bearing runs, confounded with the provider adapter; the gated arms are post hoc and select tests from benchmark metadata; Live resolve is near floor; runs are ten times shorter than public-scaffold trajectories [32, 33, 34]; intervals ignore repository clustering; event counts are lower bounds; a public-scaffold run [9, 10] is next.
