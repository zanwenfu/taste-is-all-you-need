...
We had first read the low event rate as a property of the benchmark; the second stack falsified that on the same instances. The pre-declared gate (event rate at least 0.30 per run, bearing fraction at least 25%) still failed for both stacks (GPT-5.6: 20% at calibration, 16.2% at full scale), so no arm here is a confirmatory pass, and the amendment making the gates conditional on the stack answered that failure. Regression frequency depended on the model stack in this single-sweep comparison; we claim no more.

### 5.4 Recovery policy: rollback keeps the final tree clean

![The five recovery arms under the GPT-5.6 stack on the same 40 instances. Contamination counts cells whose final patch fails a previously-passing test.](fig_contrast.pdf){width=0.82}
...
All measurements come from a 40-instance development slice, one sweep per arm, with variability measured only for plain rollback under GPT-5.6. The cross-stack comparison is six bearing runs, confounded with the provider adapter; the gated arms are post hoc. The held-out tests observe roughly the gold test patch's blast radius, so event counts are lower bounds; intervals ignore repository clustering (16 of 40 instances are django); and the instrument has not been applied to a public scaffold, which is the next experiment.
