...
def test_sibling_modules_do_not_cover_a_probe_in_another_module() -> None:
    ...
    assert covers(command, output, name, targets) == (False, "")
    # the module that does hold it counts
    assert covers("python tests/runtests.py queries --settings=tests.test_sqlite", output, name, targets)[0]
...
def test_a_k_filter_that_deselects_the_probe_is_not_coverage() -> None:
    ...
    assert covers(command, deselected, name, targets) == (False, "")
    # the tests the filter did select are covered
    name2, targets2 = parse_probe(MPL_PROBE)
    assert covers(command, deselected, name2, targets2)[0]
...
def test_the_window_ends_at_recovery() -> None:
    ...
    assert row["tested"] and row["covered"] and row["shown"]
    ...
    assert not row2["tested"] and not row2["covered"], "a post-repair run must not count"
...
def test_an_unrecovered_episode_scans_to_the_end() -> None:
    ...
    assert row["persisted_to_end"] and row["tested"] and row["covered"] and not row["shown"]
