...
def covers(command: str, output: str, name: str, probe_targets: list[str]) -> tuple[bool, str]:
    if name and name in output:
        return True, "output names the test"
    ...
    if not targets:
        return True, "whole suite"
    for target in targets:
        if any(_label_match(target, probe) for probe in probe_targets):
            return True, f"targeted {target}"
    return False, ""
...
def analyse_episode(ep: dict, commands: list[str], outputs: list[str], seqmap: dict[int, int]) -> dict:
    ...
    for i in range(start, min(end, len(commands))):
        ...
        hit, kind = covers(command, output, name, probe_targets)
        if hit:
            if not covered:
                covered, cover_kind, first_cover = True, kind, i + 1
            if names_failure(output, name) and first_shown is None:
                shown, first_shown = True, i + 1
    return {
        "probe": probe,
...
def main() -> int:
    ...
    print(f"{'instance':30s} {'probe':46s} {'onset':>5s} {'brk':>4s} {'runs':>4s} "
          f"{'tested':>6s} {'cov':>5s} {'shown':>5s}  how")
    for r in rows:
        print(f"{r['instance']:30s} {r['probe'][:46]:46s} {r['onset_cmd']:>5d} {r['commands_broken']:>4d} "
              f"{r['test_runs_in_window']:>4d} {r['tested']!s:>6s} {r['covered']!s:>5s} "
              f"{r['shown']!s:>5s}  {r['cover_kind']}{'' if r['locatable'] else ' [probe id carries no path]'}")

    incidents: dict[str, list[dict]] = {}
    for r in rows:
        incidents.setdefault(r["incident"], []).append(r)
    def n(field, source):
        return sum(1 for x in source if x[field])
    inc = [{"tested": any(x["tested"] for x in g), "covered": any(x["covered"] for x in g),
            "shown": any(x["shown"] for x in g), "persisted_to_end": any(x["persisted_to_end"] for x in g)}
           for g in incidents.values()]
    print(f"\nepisodes {len(rows):3d}: tested {n('tested', rows)}, covered {n('covered', rows)}, "
          f"shown {n('shown', rows)}, persisted {n('persisted_to_end', rows)}, "
          f"unlocatable probe ids {sum(1 for r in rows if not r['locatable'])}")
    print(f"incidents {len(inc):3d}: tested {n('tested', inc)}, covered {n('covered', inc)}, "
          f"shown {n('shown', inc)}, persisted {n('persisted_to_end', inc)}")
